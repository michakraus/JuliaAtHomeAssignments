...
struct Equation{DT <: Number, FT <: Function, FPRE <: Union{Function,Nothing}, FPOST <: Union{Function,Nothing}}
    ...
    function Equation(f::FT, x₀::Vector{Vector{DT}}; f_preproc::FPRE=nothing, f_postproc::FPOST=nothing) where {DT, FT, FPRE, FPOST}
        new{DT, FT, FPRE, FPOST}(f, Array(x₀), f_preproc, f_postproc)
    end
end

ndims(equ::Equation) = length(equ.x₀[1])
nsamples(equ::Equation) = length(equ.x₀)
